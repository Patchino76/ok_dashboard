...
sequenceDiagram
    ...
    U->>UI: Clicks Send Analyze Mill 8
    UI->>API: POST api v1 agentic analyze
    API->>API: Generate analysis_id ab12cd34
    API->>BG: asyncio create_task run_analysis_background
    API-->>UI: 200 OK analysis_id ab12cd34 status running

    Note over UI: UI starts polling GET status every 4s

    BG->>MCP: streamable_http_client session initialize
    BG->>MCP: call_tool set_output_directory
    BG->>MCP: get_mcp_tools session 9 LangChain tools
    BG->>LG: build_graph tools api_key
    BG->>LG: graph.ainvoke messages HumanMessage prompt

    Note over LG: Phase 1 LOAD
    LG->>MCP: call_tool query_mill_data mill 8
    MCP->>DB: SELECT FROM mills mill_08
    DB-->>MCP: 50000 raw SQL rows
    MCP->>MCP: pd.DataFrame rows store in dataframes
    MCP-->>LG: ToolMessage Loaded 50000 rows

    Note over LG: Phase 2 PLAN
    LG->>LG: planner node Gemini decides specialists
    LG-->>LG: stages_to_run analyst anomaly_detective

    Note over LG: Phase 3 ANALYZE
    LG->>LG: specialist analyst
    LG->>MCP: call_tool execute_python df.describe
    MCP->>MCP: exec code in namespace
    MCP->>MCP: plt.savefig chart
    MCP-->>LG: ToolMessage Chart saved mill_8_summary.png
    LG->>LG: manager_review Accept or rework
    LG->>LG: specialist anomaly_detective
    LG->>MCP: call_tool execute_python isolation_forest
    MCP->>MCP: exec code in same namespace
    MCP-->>LG: ToolMessage Anomalies detected 3 rows
    LG->>LG: manager_review Accept

    Note over LG: Phase 4 WRITE
    LG->>LG: code_reviewer checks chart quality
    LG->>MCP: call_tool write_markdown_report
    MCP->>MCP: Save report.md to output ab12cd34
    MCP-->>LG: ToolMessage Report saved report.md
    LG-->>BG: final_state messages status

    BG->>API: analyses ab12cd34 completed
    API-->>UI: GET status ab12cd34 status completed
    ...
    UI->>U: Renders report and chart in chat bubble
```
