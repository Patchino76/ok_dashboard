...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${apiUrl}/api/tag-value/${tagId}`;
    ...
    const response = await fetch(backendUrl, {
      ...
      cache: 'no-store',
      // Increase timeout for potentially slow backend responses
    ...
    console.error("Error fetching tag value:", errorMessage);
    return NextResponse.json(
      { error: "Failed to fetch tag value", details: errorMessage },
      { status: 500 }
    );
  }
}
