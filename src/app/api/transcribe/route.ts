...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log(
      `[Transcribe] Audio file: ${audioFile.name}, size: ${audioFile.size} bytes, type: ${audioFile.type}`,
    );
    ...
    if (!validTypes.includes(baseMimeType)) {
      console.error(
        `[Transcribe] Invalid audio type: ${audioFile.type} (base: ${baseMimeType})`,
      );
      return NextResponse.json(
        { error: `Invalid audio type: ${baseMimeType}` },
        { status: 400 },
      );
    }
    ...
    console.log(
      `[Transcribe] Transcription successful, length: ${data.text.length}`,
    );
