...
export function useAllMillsDowntimeData(
  ...
  const processedData = (() => {
    ...
    // Detect downtime events for each mill from real data only
...
export function useMillDowntimeData(
  ...
  const processedData = (() => {
    ...
    // Detect events from real data only - no mock data fallback
    const events = detectDowntimeEvents(millId, trendArray, config);
    const feedRateData = getMillFeedRateData(millId, trendArray);
