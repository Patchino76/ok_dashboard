  ...
  const correlations = useMemo((): CorrelationData[] => {
    ...
    for (let i = 0; i < mills.length; i++) {
      for (let j = i + 1; j < mills.length; j++) {
        ...
        const mean1 =
          values1.slice(0, n).reduce((sum, val) => sum + val, 0) / n;
        const mean2 =
          values2.slice(0, n).reduce((sum, val) => sum + val, 0) / n;
    ...
    <div className="h-full flex flex-col overflow-auto p-6 bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-lg shadow-lg p-6 border-l-4 border-green-500">
          ...
          <div className="text-3xl font-bold text-green-700 mb-2">
            {millsNames[
              parseInt(bestPerformer?.millName.replace(/\D/g, "")) - 1
            ]?.bg || bestPerformer?.millName}
          </div>
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {bestPerformer?.stability.toFixed(1)}%
              </span>
            </div>
            <div className="flex justify-between">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg shadow-lg p-6 border-l-4 border-blue-500">
          ...
          <div className="space-y-1 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {millPerformances.filter((p) => p.score > 80).length}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Под 60 резултат:</span>
              <span className="font-semibold text-orange-600">
                {millPerformances.filter((p) => p.score < 60).length}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-50 to-red-50 rounded-lg shadow-lg p-6 border-l-4 border-orange-500">
          ...
          <div className="text-3xl font-bold text-orange-700 mb-2">
            {millsNames[
              parseInt(worstPerformer?.millName.replace(/\D/g, "")) - 1
            ]?.bg || worstPerformer?.millName}
          </div>
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">Резултат:</span>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">
                {
                  anomalies.filter(
                    (a) => a.millName === worstPerformer?.millName
                  ).length
                }
              </span>
            </div>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            {millPerformances.slice(0, 6).map((perf, index) => {
              ...
              const displayName =
                millsNames[millNumber - 1]?.bg || perf.millName;
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      Средно: {perf.mean} • Стабилност:{" "}
                      {perf.stability.toFixed(1)}%
                    </div>
                  </div>
                  <div className="flex-shrink-0">
                    <div
                      className={`px-3 py-1 rounded-full text-white font-bold text-sm ${scoreColor}`}
                    >
                      {perf.score}
                    </div>
                  </div>
          ...
          {anomalies.length > 0 ? (
            <div className="space-y-2">
              {anomalies.slice(0, 6).map((anomaly, index) => {
                const millNumber = parseInt(
                  anomaly.millName.replace(/\D/g, "")
                );
                const displayName =
                  millsNames[millNumber - 1]?.bg || anomaly.millName;
                ...
                return (
                  <div
                    key={index}
                    className={`p-3 rounded-lg border ${severityColor}`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-xs mt-1">
                          Стойност: {anomaly.value} {parameterInfo?.unit} •
                          Отклонение: {anomaly.deviation}σ
                        </div>
                      </div>
                      <div className="text-xs text-gray-500">
            ...
            <div className="text-center py-8 text-gray-500">
              ...
              <div className="text-xs mt-1">
                Всички стойности са в нормални граници
              </div>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <p className="text-sm text-gray-600 mb-4">
          Мелници с силна корелация (|r| {">"} 0.5) - тенденцията им да се
          движат заедно
        </p>
