...
export const StatisticsTab: React.FC<StatisticsTabProps> = ({ parameter, timeRange, millsData }) => {
  ...
  const millStatistics = useMemo((): MillStats[] => {
    ...
    return Object.entries(millValues).map(([millName, values]) => {
  ...
  const histogramData = useMemo(() => {
    ...
    return bins.map((count, index) => ({
      range: `${(statistics.min + index * binSize).toFixed(1)}-${(statistics.min + (index + 1) * binSize).toFixed(1)}`,
      count
    }));
  ...
  if (!statistics || !millsData) {
    return (
      <div className="p-4 h-full flex items-center justify-center">
        <p className="text-gray-500">Няма налични данни за статистически анализ</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col overflow-auto p-6 bg-gray-50">
      {/* Header */}
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Статистически анализ</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Средна стойност</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.mean}</div>
          <div className="text-xs text-gray-500 mt-1">{parameterInfo?.unit}</div>
        </div>
        
        {/* Median */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Медиана</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.median}</div>
          <div className="text-xs text-gray-500 mt-1">{parameterInfo?.unit}</div>
        </div>
        
        {/* Std Dev */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-orange-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Стандартно отклонение</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.stdDev}</div>
          <div className="text-xs text-gray-500 mt-1">{parameterInfo?.unit}</div>
        </div>
        
        {/* CV */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Коефициент на вариация</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.cv}%</div>
          <div className="text-xs text-gray-500 mt-1">Вариабилност</div>
        </div>
        
        {/* Min */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Минимум</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.min}</div>
          <div className="text-xs text-gray-500 mt-1">{statistics.minMill}</div>
        </div>
        
        {/* Max */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-emerald-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Максимум</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.max}</div>
          <div className="text-xs text-gray-500 mt-1">{statistics.maxMill}</div>
        </div>
        
        {/* Range */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-indigo-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Диапазон</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.range}</div>
          <div className="text-xs text-gray-500 mt-1">{parameterInfo?.unit}</div>
        </div>
        
        {/* IQR */}
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-teal-500">
          <div className="text-xs text-gray-500 uppercase font-semibold mb-1">Междуквартилен размах</div>
          <div className="text-2xl font-bold text-gray-800">{statistics.iqr}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Разпределение на стойностите</h3>
            ...
            {histogramData.map((bin, index) => {
              ...
              return (
                <div key={index} className="flex items-center">
                  ...
                  <div className="w-12 text-xs text-gray-600 text-right">{bin.count}</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="mt-6 pt-6 border-t">
            <h4 className="text-sm font-semibold text-gray-700 mb-3">Box Plot</h4>
            <div className="relative h-16 flex items-center">
              <div className="absolute w-full h-1 bg-gray-300" style={{ top: '50%' }} />
              ...
              <div 
                ...
                style={{ 
                  left: `${((statistics.median - statistics.min) / (statistics.max - statistics.min)) * 100}%`,
                  top: '12.5%'
                }}
              />
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Производителност по мелници</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b">
                  <th className="text-left py-2 px-2 font-semibold text-gray-700">Мелница</th>
                  <th className="text-right py-2 px-2 font-semibold text-gray-700">Средно</th>
                  <th className="text-right py-2 px-2 font-semibold text-gray-700">Стабилност</th>
                  <th className="text-center py-2 px-2 font-semibold text-gray-700">Тренд</th>
                </tr>
              </thead>
              <tbody>
                {millStatistics.map((mill, index) => {
                  const millNumber = parseInt(mill.millName.replace(/\D/g, ''));
                  const displayName = millsNames[millNumber - 1]?.bg || mill.millName;
                  
                  // Color code based on stability (lower is better)
                  let stabilityColor = 'bg-green-100 text-green-800';
                  if (mill.stability > 10) stabilityColor = 'bg-yellow-100 text-yellow-800';
                  if (mill.stability > 20) stabilityColor = 'bg-red-100 text-red-800';
                  ...
                  return (
                    <tr key={index} className="border-b hover:bg-gray-50">
                      ...
                      <td className="text-right py-2 px-2">
                        <span className={`px-2 py-1 rounded text-xs ${stabilityColor}`}>
                          {mill.stability.toFixed(1)}%
                        </span>
                      </td>
                      <td className="text-center py-2 px-2">
                        {mill.trend === 'up' && <span className="text-green-600 text-lg">↑</span>}
                        {mill.trend === 'down' && <span className="text-red-600 text-lg">↓</span>}
                        {mill.trend === 'stable' && <span className="text-gray-600 text-lg">→</span>}
                      </td>
                    </tr>
                  );
                })}
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Допълнителна статистика</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Квартили</h4>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Q1 (25%):</span>
                <span className="font-medium">{statistics.q1} {parameterInfo?.unit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Q2 (50% - Median):</span>
                <span className="font-medium">{statistics.median} {parameterInfo?.unit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Q3 (75%):</span>
                <span className="font-medium">{statistics.q3} {parameterInfo?.unit}</span>
              </div>
            </div>
          </div>
          
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Разсейване</h4>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium">{statistics.stdDev} {parameterInfo?.unit}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div>
            <h4 className="text-sm font-semibold text-gray-700 mb-2">Екстремни стойности</h4>
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Минимум:</span>
                <span className="font-medium">{statistics.min} {parameterInfo?.unit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Максимум:</span>
                <span className="font-medium">{statistics.max} {parameterInfo?.unit}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Диапазон:</span>
                <span className="font-medium">{statistics.range} {parameterInfo?.unit}</span>
              </div>
            </div>
          </div>
        </div>
