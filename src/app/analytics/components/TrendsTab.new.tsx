  ...
  const processedData = React.useMemo(() => {
    ...
    if (trendData && trendData.length > 0) {
      ...
      sortedTimestamps.forEach(timestamp => {
        ...
        trendData.forEach(mill => {
          if (selectedMills[mill.mill_name]) {
            const index = mill.timestamps.findIndex(ts => ts === timestamp);
            if (index !== -1) {
              dataPoint[mill.mill_name] = mill.values[index];
            }
          }
        });
    ...
    <div className="h-full flex flex-col p-4 space-y-4 overflow-hidden">
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        {millsNames.map((mill, index) => (
          <div key={`mill-${index}-${mill}`} className="flex items-center">
            <input
              type="checkbox"
              id={`mill-${mill}`}
              checked={!!selectedMills[mill]}
              onChange={() => handleMillToggle(mill)}
              className="mr-2"
            />
            <label 
              htmlFor={`mill-${mill}`} 
