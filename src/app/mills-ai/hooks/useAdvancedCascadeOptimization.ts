...
export function useAdvancedCascadeOptimization(): UseAdvancedCascadeOptimizationReturn {
  ...
  const getOptimizationStatus = useCallback(
    async (jobId: string): Promise<OptimizationJob | null> => {
      try {
        const response = await mlApiClient.get(
          `/cascade/optimization/status/${jobId}`
        );
  ...
  const getOptimizationResults = useCallback(
    async (jobId: string): Promise<OptimizationResults | null> => {
      try {
        const response = await mlApiClient.get(
          `/cascade/optimization/results/${jobId}`
        );
  ...
  const getRecommendations = useCallback(
    async (
      ...
      try {
        ...
        const response = await mlApiClient.get(
          `/cascade/recommendations?${params}`
        );
        return response.data;
      } catch (err) {
  ...
  const listOptimizationJobs = useCallback(async (): Promise<
    OptimizationJob[]
  > => {
    try {
      ...
      const poll = async () => {
        try {
          ...
          if (status.status === "completed") {
            ...
            if (results) {
              toast.success(
                `Optimization completed! Best value: ${
                  results.best_value?.toFixed(3) || "N/A"
                }`
              );
            }
