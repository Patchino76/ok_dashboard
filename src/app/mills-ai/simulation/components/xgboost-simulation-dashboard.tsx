...
export default function XgboostSimulationDashboard() {
  ...
  const handleModelChange = async (newModelName: string) => {
    if (newModelName === modelName) return; // Skip if no change
    
    console.log('Model selection changed from', modelName, 'to', newModelName);
    ...
    try {
      ...
      if (!models || !models[newModelName]) {
        throw new Error(`Model not found: ${newModelName}`);
      }
      
      const selectedModel = models[newModelName];
      console.log('Updating model metadata for:', newModelName, selectedModel);
  ...
  const handleMillChange = (millNumber: number) => {
    ...
    if (models) {
      ...
      if (modelForMill) {
        handleModelChange(modelForMill);
      }
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-cyan-100 dark:from-blue-900/20 dark:via-indigo-800/20 dark:to-cyan-700/20 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <CardContent>
          ...
          {/* Model info summary is now part of the ModelSelection component */}
        </CardContent>
      </Card>
