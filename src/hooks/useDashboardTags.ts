...
async function fetchTagValue(tagName: string): Promise<TagValue> {
  ...
  try {
    ...
    const response = await fetch(`/api/tag-value/${tag.id}`, {
      headers: {
    ...
    try {
      ...
      if (data && (data.value !== undefined || data.timestamp)) {
        console.log(
          `Retrieved data for tag ${tag.id} despite status ${response.status}`,
        );
      } else if (!response.ok) {
        // Only throw if we don't have usable data
        throw new Error(
          `Failed to fetch tag ${tag.id}: ${response.statusText}`,
        );
      }
    } catch (parseError) {
    ...
    const value =
      tag.unit === "bool" && typeof data.value === "number"
        ? Boolean(data.value)
        : data.value;
...
export function useDashboardTags(tagNames: string[], options = {}) {
  ...
  dashboardTags
    ...
    .forEach((tag) =>
      tag.state?.forEach((stateTag) => tagsToFetch.add(stateTag)),
    );
  ...
  const data = Array.from(tagsToFetch).reduce(
    (acc, tagName, index) => {
