...
Цялата технологична верига – всички корпуси – в единна среда, в реално време. Всяка роля получава релевантната за нея информация, но всички работят с **един и същ обективен набор от данни**.
...
Към експертния опит се добавя **обективен количествен анализ и препоръки за целеви уставки към АСУТП**. Интелигентните анализи водят до по-добър избор на работни режими и по-дълбоко разбиране на процеса; експертният опит остава ценен, но вече подкрепен с факти и изчисления.
...
| Сменна вариация в решенията              | Единни, обективни критерии за управление    |
| Ръчно, бавно изготвяне на отчети         | Автоматизирани аналитични отчети за минути  |
| Неоползотворени данни                    | Данните се превръщат в решения и задания    |
...
### 4.1. Повишаване на металургичното извличане
...
По-висок **преработен тонаж (t/h)** при намалени непланирани престои и по-висок коефициент на техническо използване, осигурени от предиктивната поддръжка и съгласуваността между корпусите.

### 4.3. По-ниски специфични разходи

По-нисък **специфичен разход на енергия (kWh/t), реагенти и мелеща среда (g/t)** за всеки тон преработена руда, без компромис с качеството.
...
- **Извличане:** металургично извличане на мед (%) и съдържание на мед в хвоста.
- **Производителност:** преработен тонаж (t/h) и коефициент на техническо използване (работно време спрямо престои).
- **Разходи:** специфичен разход на енергия (kWh/t) и на реагенти (g/t).
- **Надеждност:** честота и продължителност на непланираните престои (MTBF/MTTR).
